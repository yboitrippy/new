	...
	!function(){ 
		return (window.gnezdo && window.gnezdo.create) ? window.gnezdo : (window.gnezdo = {
								...
								fpPromise.then(function(fp) {
									...
									xhr.onreadystatechange = function(){
										...
										if (xhr.status != 200){
											...
											if(!self['reload_'+p.containerId] && (p.tizerId == 326829 || p.tizerId == 326871 || p.tizerId == 326936 || p.tizerId == 330867 || p.tizerId == 331464 || p.tizerId == 331995 || p.tizerId == 331996 || p.tizerId == 333134 || p.tizerId == 333334 || p.tizerId == 333340 || p.tizerId == 333342 || p.tizerId == 333348 || p.tizerId == 333451 || p.tizerId == 333452 || p.tizerId == 333453 || p.tizerId == 333456 || p.tizerId == 333584 || p.tizerId == 333610 || p.tizerId == 333615 || p.tizerId == 333656 || p.tizerId == 333767 || p.tizerId == 333766 || p.tizerId == 333765 || p.tizerId == 333764 || p.tizerId == 333763 || p.tizerId == 333782 || p.tizerId == 333784 || p.tizerId == 333824 || p.tizerId == 333825 || p.tizerId == 333826 || p.tizerId == 333828 || p.tizerId == 333977 || p.tizerId == 333976 || p.tizerId == 333975 || p.tizerId == 334132 || p.tizerId == 334133 || p.tizerId == 334134 || p.tizerId == 334162 || p.tizerId == 334245 || p.tizerId == 334275 || p.tizerId == 334382 || p.tizerId == 334383 || p.tizerId == 334392 || p.tizerId == 334443 || p.tizerId == 334978)) {
						...
						function loader(data){
							...
							if (data){
								...
								if(data.sticky){
									...
									// фуллскрин
									if(p.tizerId == 326830 || p.tizerId == 326872 || p.tizerId == 330866 || p.tizerId == 331410 || p.tizerId == 332207 || p.tizerId == 333142 || p.tizerId == 333147 || p.tizerId == 333148 || p.tizerId == 333149 || p.tizerId == 333334 || p.tizerId == 333349 || p.tizerId == 333457 || p.tizerId == 333508 || p.tizerId == 333772 || p.tizerId == 333771 || p.tizerId == 333770 || p.tizerId == 333769 || p.tizerId == 333768 || p.tizerId == 333785 || p.tizerId == 334161 || p.tizerId == 334246 || p.tizerId == 334343 || p.tizerId == 334426 || p.tizerId == 334425 || p.tizerId == 334424 || p.tizerId == 334423 || p.tizerId == 334420 || p.tizerId == 334440 || p.tizerId == 334827 || p.tizerId == 334977 || p.tizerId == 335046 || p.tizerId == 334898 || p.tizerId == 335117 || p.tizerId == 335115 || p.tizerId == 335025) {
									...
									if (idocument && (p.noframe || idocument.querySelector("html"))){
										...
										function render(){																
											...
											if (height && height>20){
												...
												//if(data.img_client_size) {
													self._replaceGnezdoImgs(data, p.noframe ? document : idocument);
													setTimeout(function() {
														self._resizeGnezdoImgs(data, p.noframe ? document : idocument, iframe);
													}, 1000);
												//}
												...
												if (bird){
													...
													if (bird_rect){
														...
														if (cur_img && cur_img.closest('.gnezdo_block') && offsetRight < 15){
															var cur_info = cur_img.closest('.gnezdo_block').querySelector('.gnezdo_info_icon');
															if (cur_info) cur_info.style.setProperty('top', '22px', 'important');
														}
													};
												};
			...
			_gnezdo_adv_info_style: "\
				...
				.gnezdo_info{position:absolute;z-index:10;display:none;left: 0;top: 0; text-align: left;right:0;background-color:#f3f3f3;color:#333;padding:2px;border:1px solid #ccc; font-family: Open Sans,Arial,Helvetica,sans-serif; font-size:10px;word-wrap: break-word;}\
			...
			_srcGnezdoImg: function(data, img) {
				var self = this;
				...
				try {
					...
					if(m && m.length>=2 && img.match(/zn4\./)) { // и проверим что zn4
						data.img_natural = { w: m[1], h: m[2] };
						window.gnezdo.imgClientSizeCache = window.gnezdo.imgClientSizeCache || {};
						if(window.gnezdo.imgClientSizeCache[m[0]]) return window.gnezdo.imgClientSizeCache[m[0]]; // смотрим в кеш
						var w = m[1], h = m[2];
						var canvas = document.createElement("canvas");
						canvas.width = w;
						canvas.height = h;
						ret = canvas.toDataURL("image/png");
						window.gnezdo.imgClientSizeCache[m[0]] = ret;
					}
				} catch(ex) {
					gnezdoErrorHandler(window.top || window.parent || window, { stack: "catch at _srcGnezdoImg: "+ex.message});
				}
			...
			_replaceGnezdoImgs: function(data, idocument) {
				var self = this;
				var imgs = idocument.getElementsByClassName("gnezdo_img");
			...
			_resizeGnezdoImgs: function(data, idocument, iframe) {
				var self = this;
				...
				iwnd.addEventListener("resize", function() {
					...
					tid = setTimeout(function() {
						self._replaceGnezdoImgs(data, idocument);
					}, 200);
				});
			},
			...
			_createContent: function(data, p){
				...
				for (var i = 0; i < data.arr.length; i++){
					...
					if (an.official_name || an.inn || an.erid){
						info = adv+'<div class="gnezdo_info_icon" onmouseover="this.nextSibling.classList.add(\'active\');" onmouseout="this.nextSibling.classList.remove(\'active\');"></div><div class="gnezdo_info">';
			...
			_createContentCustom: function(data, p){
				...
				for (var i = 0; i < data.arr.length; i++){
					...
					html += '<div class="'+an.cell_class+'">'+info+'<div class="gnezdo_block_inner"><a rel="nofollow '+(an.gnezdo_url?'noreferrer':'')+'" rtb="'+(an.rtb?an.rtb:0)+'" href="'+an.url+'" '+(an.gnezdo_url?' gnezdo_href="'+an.gnezdo_url+'" onmousedown="return parent.gnezdo._gnezdoClickCheck(event , this);"':'')+' target="_blank"><div class="gnezdo_block_img_wrap"><picture><img '+(an.rtb&&k_xy?'onload="if (this.parentElement && this.parentElement.parentElement) this.parentElement.parentElement.style.maxHeight = this.width/'+k_xy+'"':'')+(data.sticky ? ' crossorigin="anonymous" ' : '' )+' loading="eager" aid="'+an.id+'" src="'+self._srcGnezdoImg(data, an.img)+'" class="gnezdo_img" alt="'+an.title+'"></picture></div><div class="gnezdo_block_desc"><div class="gnezdo_block_desc_bg"></div><div class="gnezdo_block_desc_text">'+an.title+'</div>'+(data.show_anons?'<div class="gnezdo_block_desc_longtext">'+an.text+'</div>':'')+an.price_block+(data.show_partner?'<div class="gnezdo_block_partner">'+(data.show_favicon?an.favicon_image:'')+an.site+'</div>':'')+(data.readmore_text?'<div class="gnezdo_readmore">'+data.readmore_text+'</div>':'')+'</div></a></div></div>';
				}
			...
			_createContentAdaptive: function(data, p){
				...
				for (var i = 0; i < data.arr.length; i++){
					...
					html += '<div class="'+cell_class+'" style="width: '+width+'; margin-right:'+(((i+1)%cols == 0)?0:1)+'%; '+(i>=cnt?"display:none;":"")+'">'+info+'<div class="gnezdo_cont_inner" style="position: relative;"><a rel="nofollow '+(an.gnezdo_url?'noreferrer':'')+'" href="'+url+'" target="_blank"><div class="gnezdo_cont_img_wrap"><picture><img  ' + (data.sticky ? ' crossorigin="anonymous" ' : '' ) +' loading="eager" style="width: 100%;" aid="'+an.id+'" src="'+self._srcGnezdoImg(data, img)+'" class="gnezdo_img" alt="'+an.title+'"></picture></div><div class="gnezdo_cont_desc"><div class="gnezdo_cont_desc_text">'+an.title+'</div>'+((show_anons && data.show_anons)?'<div class="gnezdo_cont_desc_longtext">'+an.text+'</div>':'')+price_block+((show_partner && data.show_partner)?'<div class="gnezdo_cont_partner">'+(data.show_favicon?an.favicon_image:'')+an.site+'</div>':'')+((show_readmore&& data.readmore_text)?'<div class="gnezdo_readmore">'+data.readmore_text+'</div>':'')+'</div></a></div></div>';
				}
			...
			_createContentSlider: function(data, p) {
				...
				html += '<div class="gnezdo_main_block"><div class="gnezdo_block_container"><div id="gnezdoComposite" style=""><div class="mgoc"><div class="mglogo"> <a href="https://www.gnezdo.ru/?news_id=' + p.tizerId + '" target="_blank" rel="nofollow noreferrer">'+(data.header_text||'')+'</a> </div><div class="mgbox MG_Gallery"><div class="swiper-container swiper-container-lenta swiper-container-horizontal"><div class="swiper-wrapper swiper-wrapper-lenta">';
				...
				html += data.arr.map(function(an) {
					...
					return '<div class="'+an.cell_class+' swiper-slide swiper-slide-lenta"> \
						<div class="mgline teaser- type-e"> '+info+'\
						 ...
						 <div class="image-container"><picture><img '+((an.rtb && img_xy[0] && img_xy[1])?' style="height: 102%; object-fit: cover;" ':'')+' loading="eager"' + ((data.gradient && !an.rtb) ? ' crossorigin="anonymous"' : '') + ' class="mcimg gnezdo_img" aid="'+an.id+'" src="'+self._srcGnezdoImg(data, an.img)+'" /></picture> <div class="mctitle"><span class="gnezdo_text">'+an.title+'</span>'+(data.show_anons?'<span class="gnezdo_longtext">'+an.text+'</span>':'')+an.price_block+(data.show_partner?'<span class="gnezdo_partner">'+(data.show_favicon?an.favicon_image:'')+an.site+'</span>':'')+'</div></div> </a> </div> </div></div>  </div>';
				}).join('');
			...
			_createContentNative: function(data, p){
				...
				for (var i = 0; i < data.arr.length; i++){
					...
					var disclaimer = '<span style="display: inline-block; outline: 0.5px solid white;"><span class="main" style="' + (an.disclaimer?'':'display:none;') + '">' + an.disclaimer + '</span>' + (an.disclaimer_add?((an.disclaimer?'<br/>':'')+'<span class="add">' + an.disclaimer_add + '</span>'):'') + '</span>';
					...
					if (data.native_box){
						...
						for (var j = 0; j < an.multis.length; j++){
							...
							if(m.image.match(/zn4\./)) {// && data.enable_webp) {
								image_video += '<source type="image/webp" srcset="' + m.image.replace(/jpg$/, 'webp') + '" />';
							}
							image_video += '<source type="image/jpeg" srcset="' + an.img + '" />\
